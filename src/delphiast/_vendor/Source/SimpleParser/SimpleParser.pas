...
begin
  ...
  try
    Self.Position := 0;
    if Self.Size > 0 then
      Self.ReadBuffer(Bytes[0], Self.Size);
  finally
  ...
  PreambleSize := Length(Encoding.GetPreamble);
  if PreambleSize > Length(Bytes) then
    PreambleSize := 0;

  Result := Encoding.GetString(Bytes, PreambleSize, Length(Bytes) - PreambleSize);
{$ENDIF}
end;
