...
type
  ...
  strict private type
    ...
    strict private
      {$IFNDEF FPC}[weak]{$ENDIF} FIncludeCache: TIncludeCache;
      {$IFNDEF FPC}[weak]{$ENDIF} FIndexer     : TProjectIndexer;
      {$IFNDEF FPC}[weak]{$ENDIF} FProblems    : TProblems;
      FUnitFile           : string;
...
begin
  ...
  if (APath[1] = PathDelim) or (APath[1] = '/') or (APath[1] = '\') then
    Exit(False);
